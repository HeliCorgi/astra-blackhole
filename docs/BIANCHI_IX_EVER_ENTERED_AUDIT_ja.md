...
GitHub ActionsでCAP強度、境界幅、block-Krylov次元を分離して走らせた。第一壁A条件付きの量子重みは0.979742。古典Wigner 4096軌道ではA条件付き3935本のうち99.8729%がs=32.46までにhard B領域へ一度以上入り、first-entryの10/50/90%分位は約5.49/6.81/9.16。ds=.01→.005でever-entry割合は変わらなかった。

量子CAPは確率として採用できなかった。width=.30の主要scanは：

| V0 | maxdim | no-B対角重み | ever-B対角重み | |Doff| | Re Doff |
|---:|---:|---:|---:|---:|---:|
| .025 | 66 | .05720 | .72202 | .11048 | +.11039 |
| .040 | 78 | .02274 | .87037 | .05358 | +.05345 |
| .050 | 78 | .01550 | .92031 | .03210 | +.03209 |
| .060 | 78 | .01259 | .95457 | .01666 | +.01642 |
| .075 | 78 | .01188 | .98980 | .00733 | -.00084 |
| .075 | 90 | .01165 | .98978 | .00578 | -.00072 |
| .085 | 78 | .01255 | 1.00605 | .01361 | -.00930 |
| .100 | 78 | .01429 | 1.02309 | .02323 | -.01869 |

V0=.075近傍でstrict off-diagonalが最小になるが、.085/.10で再増大するため広いregulator plateauではない。strong CAPでno-B重み自体も再増加する挙動はcomplex-potential法で知られる反射/Zeno側への移行と整合的だが、この模型で反射を独立測定した結果ではない。

V0=.075,maxdim78でwidth=.20/.30/.45を変えると |Doff|=.00748/.00733/.00711 とstrict量は比較的安定する一方、Re Doff=-.00135/-.00084/+.00020 とゼロを横切る。したがってweak/real consistencyも「成立」と採用しない。V0=.05のmaxdim66→78で |Doff| 差は.00190、V0=.075の78→90差は.00155。CAP強度依存の方が大きい。

**結論：finite-clock CAP版のever-entered operatorは実装できたが、regulator-independentなデコヒーレンス窓を確認できなかった。対角重みをever-B確率とは呼ばない。** 特に古典99.87%に近いV0=.075の対角重み約.99を選ぶことは、regulatorを望む結論へ合わせることになるので禁止する。

次はこのpilotを負の対照として固定し、timeless WDW constraintと可換なHalliwell型S-matrix class operatorへ進む。finite-clock版を再検査する場合は、CAP反射をincoming/outgoing fluxで独立診断する。
